...
#
# The script assumes ARM CPU, so Function ("f") symbols get rounded to even numbers and if address points to uninitialized space -- marks it as word,
# making thumb calls handle labels correctly.
#
# @author unkown; edited by matedealer <git@matedealer.de>; then Anton Fedorov <datacompboy@gmail.com>
# @category Data
#
...
for line in file(f.absolutePath):  # note, cannot use open(), since that is in GhidraScript
    ...
    if function_or_label == "f":
        ...
        if func is not None:
            ...
            address=address.subtract(address.getOffsetAsBigInteger()%2)  # Fix thumb
            func = createFunction(address, name)
            print("Created function {} at address {}".format(name, address))
            
            if str(currentProgram.getListing().getCodeUnitAt(address)) == "?? ??":
                currentProgram.getListing().createData(address, ghidra.program.model.data.WordDataType())
